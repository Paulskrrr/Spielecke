...
(function (global) {
  ...
  var DE = {
    ...
    "Every wire is live. Sever one whose charge has nowhere to go and the device fires — a conductor is only safe to cut once you have opened it a path. That path is the CHANNEL, and you steer it with the Dials.": "Jeder Draht steht unter Strom. Durchtrennst du einen, dessen Ladung nirgendwohin kann, zündet das Gerät — ein Leiter ist erst dann sicher zu schneiden, wenn du ihm einen Weg geöffnet hast. Dieser Weg ist der KANAL, und du steuerst ihn mit den Reglern.",
    "The CHANNEL is where the Dials send the charge: <b>Dial A + Dial B</b>. Set both dials to their targets first (see Dials), then add them — dials on 4 and 5 open channel 9. The wire carrying that channel is now the safe one.": "Der KANAL ist, wohin die Regler die Ladung schicken: <b>Regler A + Regler B</b>. Stell beide Regler zuerst auf ihre Zielwerte (siehe Regler) und addiere sie — Regler auf 4 und 5 öffnen Kanal 9. Der Draht, der diesen Kanal führt, ist jetzt der sichere.",
    "Cut the wire whose printed number equals the channel. If several carry it, only the leftmost is truly discharged — cut that one.": "Durchtrenne den Draht, dessen aufgedruckte Zahl dem Kanal entspricht. Führen mehrere ihn, ist nur der am weitesten links wirklich entladen — den durchtrennst du.",
    "If no wire's number matches the channel, the charge overflows onto the colour bus: cut the highest-ranked colour on the Decoder's priority list (1 = highest). Ties go to the leftmost.": "Passt keine Zahl zum Kanal, läuft die Ladung auf den Farb-Bus über: durchtrenne die höchstplatzierte Farbe aus der Prioritätsliste des Decoders (1 = höchste). Bei Gleichstand den am weitesten links.",
    "Check the firing order before you cut — a wire severed out of sequence jams the interlock (a strike). See Firing order.": "Gleiche vor dem Schnitt die Zünd-Reihenfolge ab — ein außer der Reihe getrennter Draht blockiert die Verriegelung (ein Fehler). Siehe Zünd-Reihenfolge.",
    ...
    "The four modules are interlocked in series at the factory: each stage physically unlocks the next, so the device cannot be undone by luck or in the wrong order. Force a stage out of sequence and the interlock jams — that is a strike. The order is set at manufacture and cannot be changed in the field.": "Die vier Module sind werkseitig in Reihe verriegelt: jede Stufe entriegelt die nächste, damit sich das Gerät nicht per Zufall oder in falscher Reihenfolge lösen lässt. Erzwingst du eine Stufe außer der Reihe, blockiert die Verriegelung — das ist ein Fehler. Die Reihenfolge ist werkseitig festgelegt und im Feld nicht änderbar.",
    "The detonator's safety only lifts for a single clock-tick each cycle — a deliberately narrow window, so a panicking operator can't just mash the button. Bleed the charge on the Dials, authorise it on the Keypad, then release on the tick.": "Die Sicherung des Zünders hebt sich pro Zyklus nur für einen einzigen Uhren-Tick — ein bewusst schmales Fenster, damit ein panischer Entschärfer nicht einfach den Knopf zerhämmert. Leite die Ladung über die Regler ab, autorisiere sie am Tastenfeld und lass genau im Tick los.",
    // Maze — keyed corner for orientation
    "One corner of the grid is clipped — a factory alignment key. Have the operator turn the bomb until that amber corner sits TOP-LEFT, so their grid matches the diagrams below (which carry the same mark).": "Eine Ecke des Gitters ist abgeschrägt — ein werkseitiger Passschlüssel. Lass den Entschärfer die Bombe drehen, bis diese gelbe Ecke OBEN LINKS sitzt, damit sein Gitter zu den Diagrammen unten passt (die dieselbe Marke tragen).",
    "Now have them read out the two ringed cells and find the diagram with rings in the SAME two cells — that is the active maze.": "Lass ihn dann die beiden umringten Zellen ansagen und finde das Diagramm mit Ringen in denselben zwei Zellen — das ist das aktive Labyrinth.",
